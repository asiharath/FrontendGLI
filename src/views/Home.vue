...
export default {
  ...
  async created() {/* eslint no-console: ["error", { allow: ["log", "error"] }] */
    ...
    const apiDriving = `https://maps.googleapis.com/maps/api/directions/json?origin=${this.start}t&destination=${this.end}&key=${gMap.apiKey}`
    const apiWalking = `https://maps.googleapis.com/maps/api/directions/json?origin=${this.start}t&destination=${this.end}&mode=walking&key=${gMap.apiKey}`
    const apiBicycling = `https://maps.googleapis.com/maps/api/directions/json?origin=${this.start}t&destination=${this.end}&mode=bicycling&key=${gMap.apiKey}`
